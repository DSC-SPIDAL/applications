...
$BUILD/bin/mpirun --hostfile $7 --mca btl ^tcp --report-bindings --map-by ppr:$ppn:node:SPAN  --bind-to $bindToUnit --rank-by core  -np $(($nodes*$ppn)) java $opts  $PWC_OPS -cp $cp edu.indiana.soic.spidal.dapwc.Program -c config.properties -n $nodes -t $tpp 2>&1 | tee $pat.$xmx.$memmultype.$4.$3.out.txt
